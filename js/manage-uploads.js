...
function rowEl(docId, v){
  ...
  el.dataset.id = docId;
  el.innerHTML = `
    <label class="selout"><input type="checkbox" class="selbox"/></label>
    <div class="row">
      <div class="info">
...
btnDeleteSel?.addEventListener('click', async ()=>{
  const ids = Array.from(document.querySelectorAll('.item .selbox:checked'))
    .map(cb => cb.closest('.item')?.dataset.id).filter(Boolean);
