// js/watch.js — 단일 코드로 iOS/Android/PC 자동 튜닝
...
function updateVh(){
  ...
  setAppVh(vv && typeof vv.height==='number' ? vv.height : window.innerHeight);
}
updateVh();
const vv = window.visualViewport;
...
function detectPlatform(){
  ...
  const qp = new URL(location.href).searchParams.get('mode'); // 디버그용 강제 모드 ?mode=ios|android|desktop
  const isiPadOS = navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1; // iPadOS가 'Mac'로 위장
  const isIOS = qp ? (qp==='ios') : (/iPad|iPhone|iPod/.test(ua) || isiPadOS);
  const isAndroid = qp ? (qp==='android') : /Android/i.test(ua);
  const isDesktop = qp ? (qp==='desktop') : (!isIOS && !isAndroid);
  return { isIOS, isAndroid, isDesktop, mode: qp || (isIOS?'ios':isAndroid?'android':'desktop') };
}
const PLATFORM = detectPlatform();

// iOS는 보수적으로, Android/PC는 더 공격적으로 사전 로딩
const SETTINGS = {
  PAGE_SIZE:        PLATFORM.isIOS ? 8  : 12,
  PRELOAD_AHEAD:    PLATFORM.isIOS ? 1  : 2,   // 다음 카드 몇 장 프리로드할지
  UNMOUNT_DISTANCE: PLATFORM.isIOS ? 2  : 3    // 현재로부터 이 이상 떨어지면 언마운트
};
...
function makeCard(url, docId, isFirst=false){
  ...
  card.dataset.mounted = '0'; // iframe 장착 여부
...
/* ---------- iframe mount/unmount (iOS 메모리 보호) ---------- */
...
function preloadAheadFrom(card){
  // 다음 N장 프리로드 (플랫폼별)
  let el = card;
  for (let k=1; k<=SETTINGS.PRELOAD_AHEAD; k++){
    el = el?.nextElementSibling;
    if (el && el.classList?.contains('video')){
      (window.requestIdleCallback ? requestIdleCallback : setTimeout)(()=> mountIframe(el, {preload:true}), 120*k);
    } else break;
  }
}
function onScroll(){
  ...
  requestAnimationFrame(()=>{
    ...
    if (centered && centered !== currentActive){
      ...
      // 다음 카드들 프리로드
      preloadAheadFrom(currentActive);

      // 원거리(±UNMOUNT_DISTANCE 이상) 언마운트로 메모리 보호
      cleanupFarFrames();
      showTopbar();
    }
    scrollTicking = false;
...
function cleanupFarFrames(){
  ...
  cards.forEach((c, i)=>{
    ...
    if (dist >= SETTINGS.UNMOUNT_DISTANCE && c.dataset.mounted === '1') unmountIframe(c);
  });
}
...
async function loadMore(initial=false){
  ...
  try{
    ...
    parts.push(limit(SETTINGS.PAGE_SIZE));
    ...
    if (snap.size < SETTINGS.PAGE_SIZE) hasMore = false;

    // 초기에는 중앙/첫 카드 즉시 장착 + 프리로드
    if (initial){
      requestAnimationFrame(()=>{
